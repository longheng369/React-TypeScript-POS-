...
const Supplier: React.FC = () => {
   ...
   const showModal = (record?: any) => {
      if (record) {
         ...
         form.setFieldsValue({ status: true }); // Set status to true for new supplier
      }
   ...
   //          `${import.meta.env.VITE_URL}/suppliers`,
   //          values
   //       );
   ...
   const editSupplier = async (values: any) => {
      try {
         await axios.put(
            `${import.meta.env.VITE_URL}/suppliers/${currentRecord.id}`,
            values
         );
         queryClient.invalidateQueries({ queryKey: ["suppliers"] });
         setIsModalVisible(false);
